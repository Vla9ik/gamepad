...
function setHash(hash2) {
  hash2 = hash2 ? `#${hash2}` : window.location.href.split("#")[0];
  history.pushState("", "", hash2);
}
...
function getBoundingClientRect(element2, includeScale, isFixedStrategy) {
  ...
  var _ref = isElement$1(element2) ? getWindow$1(element2) : window, visualViewport = _ref.visualViewport;
...
if (document.querySelector("[data-fls-tippy-info]")) {
  window.TIPPY_INFO = tippy("[data-fls-tippy-info]", {
    allowHTML: true,
    interactive: false,
    hideOnClick: true,
    appendTo: () => document.body,
...
function scrollFromHashSimple() {
  const raw = window.__pendingHash || (location.hash ? decodeURIComponent(location.hash).replace(/^#/, "") : "");
  if (!raw) return;
  const selector3 = document.getElementById(raw) ? `#${raw}` : document.querySelector(`.${raw}`) ? `.${raw}` : null;
  if (!selector3) {
    let tries = 30;
    const wait = () => {
      const s = document.getElementById(raw) ? `#${raw}` : document.querySelector(`.${raw}`) ? `.${raw}` : null;
      if (s) {
        setTimeout(() => {
          gotoBlock(s, false, 500, 0);
        }, 300);
      } else if (tries-- > 0) {
  ...
  setTimeout(() => {
    gotoBlock(selector3, false, 500, 0);
  }, 1e3);
}
...
(function() {
  ...
  window.__pendingHash = decodeURIComponent(location.hash).replace(/^#/, "");
...
function instance($$self, $$props, $$invalidate) {
  ...
  function sliderFocusHandle(event) {
    ...
    if (!disabled) {
      $$invalidate(36, activeHandle = elementIndex$1(target));
  ...
  function sliderKeydown(event) {
    if (!disabled) {
      let prevent = false;
      const handle = elementIndex$1(event.target);
      let jump = step;
  ...
  $$self.$$set = ($$props2) => {
    ...
    if ("class" in $$props2) $$invalidate(28, classes2 = $$props2.class);
...
function initSliders$1(onChange) {
...
function elementChildren(element2, selector3) {
  ...
  if (window2.HTMLSlotElement && element2 instanceof HTMLSlotElement) {
    children2.push(...element2.assignedElements());
  }
  if (!selector3) {
    return children2;
  }
  return children2.filter((el) => el.matches(selector3));
}
  ...
  if (swiper.params.createElements) {
    Object.keys(checkProps).forEach((key) => {
      if (!params[key] && params.auto === true) {
        ...
        if (!element2) {
          element2 = createElement$1("div", checkProps[key]);
...
function tabs() {
  ...
  if (tabs2.length > 0) {
    const hash2 = getHash();
    if (hash2 && hash2.startsWith("tab-")) {
      tabsActiveHash = hash2.replace("tab-", "").split("-");
    }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const modeButtons = [btnComments, btnDescription, btnBoth].filter(Boolean);
  const showCombosSwitch = document.querySelector("#tabs");
  const colorblindSwitch = document.querySelector("#bg");
  function setMode(mode) {
    html2.dataset.addonsMode = mode;
    html2.classList.toggle("comments-modal", mode === "both");
    modeButtons.forEach((b) => b?.classList.remove("active"));
    if (mode === "comments") btnComments?.classList.add("active");
    if (mode === "description") btnDescription?.classList.add("active");
    if (mode === "both") btnBoth?.classList.add("active");
    const enableTips = mode === "comments";
    (window.TIPPY_INFO || []).forEach((inst) => {
      if (!inst) return;
      if (enableTips) inst.enable();
      else inst.disable();
    });
    const allowPopups = mode === "description" || mode === "both";
    document.querySelectorAll("[data-fls-popup-link]").forEach((el) => {
      if (!allowPopups) {
        el.setAttribute("aria-disabled", "true");
        el.classList.add("--popup-disabled");
      } else {
        el.removeAttribute("aria-disabled");
        el.classList.remove("--popup-disabled");
      }
    });
  }
  document.addEventListener(
    "click",
    (e) => {
      const trigger = e.target.closest?.("[data-fls-popup-link]");
      if (!trigger) return;
      const mode = html2.dataset.addonsMode || "comments";
      const allow = mode === "description" || mode === "both";
      if (!allow) {
        e.preventDefault();
        e.stopImmediatePropagation();
      }
  ...
  btnComments?.addEventListener("click", () => setMode("comments"));
  btnDescription?.addEventListener("click", () => setMode("description"));
  btnBoth?.addEventListener("click", () => setMode("both"));
  if (showCombosSwitch) {
    const applyCombosVisibility = (on) => {
      document.querySelectorAll("[data-fls-tabs]").forEach((el) => {
        el.hidden = !on;
      });
    };
    applyCombosVisibility(showCombosSwitch.checked);
    showCombosSwitch.addEventListener("change", () => {
      applyCombosVisibility(showCombosSwitch.checked);
    });
  }
  if (colorblindSwitch) {
    const applyColorblind = (on) => html2.classList.toggle("colorblind", on);
    applyColorblind(colorblindSwitch.checked);
...
window.addEventListener("load", () => {
  document.querySelectorAll(".addons__items").forEach((container) => {
    const tabBlocks = Array.from(container.querySelectorAll("[data-fls-tabs]"));
    tabBlocks.forEach((currentBlock) => {
      currentBlock.addEventListener("click", (e) => {
        const titleEl = e.target.closest("[data-fls-tabs-title]");
        if (!titleEl || !currentBlock.contains(titleEl)) return;
        const currentTitles = Array.from(currentBlock.querySelectorAll("[data-fls-tabs-title]")).filter((el) => el.closest("[data-fls-tabs]") === currentBlock);
        const clickedIndex = currentTitles.indexOf(titleEl);
        if (clickedIndex < 0) return;
        tabBlocks.forEach((otherBlock) => {
          if (otherBlock === currentBlock) return;
          const otherTitles = Array.from(otherBlock.querySelectorAll("[data-fls-tabs-title]")).filter((el) => el.closest("[data-fls-tabs]") === otherBlock);
          const otherItems = Array.from(otherBlock.querySelectorAll("[data-fls-tabs-item]")).filter((el) => el.closest("[data-fls-tabs]") === otherBlock);
          if (!otherTitles.length || otherTitles.length !== currentTitles.length) return;
          otherTitles.forEach((t) => t.classList.remove("--tab-active"));
          if (otherTitles[clickedIndex]) {
            otherTitles[clickedIndex].classList.add("--tab-active");
          }
          otherItems.forEach((item, idx) => {
            item.classList.remove("--slide");
            item.hidden = idx !== clickedIndex;
